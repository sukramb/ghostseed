...
export type SidebarItem =
  | { name: React.ReactNode; href: string; icon: LucideIcon; type: "item" }
  | { type: "separator" }
  | { name: React.ReactNode; type: "label" };

function NavItem(props: {
  item: Extract<SidebarItem, { type: "item" }>;
  onClick?: () => void;
  basePath: string;
}) {
...
function HeaderBreadcrumb(props: { items: SidebarItem[]; baseBreadcrumb?: HeaderBreadcrumbItem[]; basePath: string }) {
  const segment = useSegment(props.basePath);
  const item = props.items.find((item) => item.type === "item" && item.href === segment) as Extract<SidebarItem, { type: "item" }> | undefined;
  const title: React.ReactNode | undefined = item?.name;
          ...
          <div className="flex md:hidden items-center">
            <Sheet onOpenChange={(open) => setSidebarOpen(open)} open={sidebarOpen}>
              ...
              <SheetContent side="left" className="w-[240px] p-0">
                <SidebarContent onNavigate={() => setSidebarOpen(false)} items={props.items} sidebarTop={props.sidebarTop} basePath={props.basePath} />
              </SheetContent>
            </Sheet>
